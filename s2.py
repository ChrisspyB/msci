# TODO: Lots of test code lying around - keep?
...
from deriv_funcs_light import E_f, rho, Delta, pomega, ray0_b_from_pos0_n0, \
    deriv as deriv_l, metric as metric_l, inv_metric as inv_metric_l
from utils import maxima, minima, xyz_to_bl, bl_to_xyz, \
    deriv_rtp_to_xyz, deriv_xyz_to_rtp

# OUTPUT PARAMS
nt = 100000
n_periods = 1.1 # number of periods to trace

# MODEL PARAMS
a = 0.99 # black hole angular momentum
M = 4.28e6 # black hole mass [solar masses]
R_0 = 8.32 # distance from earth [kpc]
spin_phi = 0
spin_theta = 0
# ^ spin orientation in observer frame (using same parameters as for orbit)
...
### Calculate orbit ###
# - Note: several intermediate parts are used inside functions later on 
...
T = n_periods * period # units of 1/2 r_s / c
...
def cast_ray(xyz0, n0, zeta, a):
    ...
    pos0 = xyz_to_bl(xyz0, a) # initial position of ray in BL coords
    mat = deriv_xyz_to_rtp(xyz0, pos0, a)
    ray0 = np.concatenate((pos0, np.zeros(2))) # initial [r, theta, phi, pr, pt]
    metric0 = metric_l(ray0, a)

    _p = np.zeros(4)
    _p[1:4] = mat @ n0
    _p[0] = (-1 - _p[3] * metric0[0, 3])/metric0[0,0] # from definition E = -p^a u_a
    _p_cov = metric0 @ _p
...
def cast_ray_freqshift(xyz0, n0, zeta, star_vel_bh, a):
    ...
    # find redshift (fully GR) #

    metric1 = metric_l(ray[-1], a) # metric at emission
    inv_metric1 = inv_metric_l(ray[-1], a)
    
    p_cov1 = np.array([-1, ray[-1, 3], ray[-1, 4], b]) # covariant four-momentum at emission
    ...
    n1 = mat1 @ p1[1:4] # cartesian dx/dt
    n1 = n1 / np.linalg.norm(n1) # (should be normalised anyway)
    ...
    u1_dt = np.ones(4) # observer dx/dt
    u1_dt[1:4] = mat1_inv @ u1_dt_xyz[1:4]
    
    dt_dtau1 = 1/np.sqrt(-(metric1 @ u1_dt) @ u1_dt) 
    u1 = dt_dtau1 * u1_dt # change dx/dt to 4-velocity

    E1 = - p_cov1 @ u1 # energy at emission
...
def cast_earth_ray_freqshift(x, y, zeta_end, star_vel_bh, a):
    ...
    z_inf = 1e7 # large compared to r_s/2, and closest distance of star to Earth
    
    xyz0 = bh_from_obs @ np.array([x,y,z_inf]) # initial position(x0,y0,z0) in obs frame
    n0 = bh_from_obs @ np.array([0, 0, 1]) # rays coming to Earth from star
    zeta = np.array([0, zeta_end]) # only need start and end points
...
def cast_earth_ray(x, y, pos_bh, nt, a):
    ...
    # time taken by ray travelling to star along straight line, units of 1/2 r_s / c
...
def deflec_and_shift(points,plot=True):

    sub_orbit = orbit[::(nt+1)//points]
    sub_t = sub_orbit[:, 0]
...
def dist_to_peri(nx=32,ny=32):
    ...
    ray, _ = cast_earth_ray(xspace[1], yspace[10], bh_from_obs @ peri_obs, 1024, a)
    ray_xyz = bl_to_xyz(ray[:, 0:3], a)
...
def test_ellipse():
    ...
    plt.plot(test_ellipse_obs[:, 0], test_ellipse_obs[:, 1], zs=test_ellipse_obs[:, 2])
    ax.scatter([0],[0])
    ax.plot(orbit_obs[:, 0], orbit_obs[:, 1], zs=orbit_obs[:, 2])
    return 
...
plt.close()
get_precession_angle()
deflec_and_shift(10,True)
plot_orbit()
dist_to_peri()
test_ellipse()
plt.show()
